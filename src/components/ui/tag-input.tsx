...
>((props, ref) => {
  ...
  if (
    ...
    console.warn(
      'maxTags and minTags cannot be less than 0',
    );
    toast({
    ...
    if (
      ...
      if (minLength && newTagText.length < minLength) {
        ...
        toast({
          ...
          description:
            'Please enter a tag with more characters',
          variant: 'destructive',
        });
        return;
      }
      ...
      if (maxLength && newTagText.length > maxLength) {
        toast({
          ...
          description:
            'Please enter a tag with less characters',
          variant: 'destructive',
        });
      ...
      if (
        newTagText &&
        (allowDuplicates ||
          !tags.some((tag) => tag.name === newTagText)) &&
  ...
  const removeTag = (idToRemove: string) => {
    ...
    onTagRemove?.(
      tags.find((tag) => tag.id === idToRemove)?.name || '',
    );
    setTagCount((prevTagCount) => prevTagCount - 1);
  };
  ...
  const filteredAutocompleteOptions = autocompleteFilter
    ? autocompleteOptions?.filter((option) =>
        autocompleteFilter(option.name),
      )
    : autocompleteOptions;
